...
#pragma warning restore CS0649 // unassigned variable
		...
		[SerializeField]
		public AudioSource readingAudio;
		
		[SerializeField]
		public AudioSource inspectingAudio;
		
		[SerializeField]
		public AudioSource searchingAudio;
		...
		private string handleRequest(HttpListenerRequest request)
		{
			...
			switch (request.Url.LocalPath.TrimEnd('/'))
			{
				...
				case "/api/soundReading":
					return playSound(readingAudio);

				case "/api/soundInspection":
					return playSound(inspectingAudio);

				case "/api/soundSearch":
					return playSound(searchingAudio);

				case "/api/lastGazeData":
					return lastGazes(request);
		...
		/// <summary>
		/// Retrieve and return the last 750 rows of the csv
		/// </summary>
		/// <param name="request"></param>
		/// <returns></returns>
		private string lastGazes(HttpListenerRequest request)
		{
			// Return result
			Success response;
			...
			{
				success = true,
				message = String.Join("", dataLogger.lastGazes.ToArray())
			};
			return JsonUtility.ToJson(response, true);
		}

		private string playSound(AudioSource source)
		{
			if (!source)
			{
				Debug.LogError("There is no source.");
			}
