...
namespace ARETT {
	...
	public class DataLogger : MonoBehaviour
	{
		...
		public ConcurrentQueue<string> lastGazes { get; private set; } = new ConcurrentQueue<string>();
		...
		private void NewDataHandler(GazeData gazeData)
		{
			...
			String content = logStringBuilder.ToString();

			// Write info to file
			FileHandler.WriteData(content);

			if (lastGazes.Count > 750) 
			{
				lastGazes.TryDequeue(out string info);
			}
			lastGazes.Enqueue(content);
		}
	}
}
